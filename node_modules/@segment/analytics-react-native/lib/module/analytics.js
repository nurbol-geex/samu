...
import { settingsCDN, workspaceDestinationFilterKey, defaultFlushInterval, defaultFlushAt } from './constants';
import { getContext } from './context';
import { createAliasEvent, createGroupEvent, createIdentifyEvent, createScreenEvent, createTrackEvent } from './events';
import { CountFlushPolicy, Observable, TimerFlushPolicy } from './flushPolicies';
...
import { allSettled, getPluginsWithFlush, getPluginsWithReset, getURL } from './util';
import { getUUID } from './uuid';
import { checkResponseForErrors, ErrorType, SegmentError, translateHTTPError } from './errors';
export class SegmentClient {
  ...
  pluginsToAdd = [];
  flushPolicyExecuter = new FlushPolicyExecuter([], () => {
    void this.flush();
  });
  ...
  getPlugins(ofType) {
    ...
    return [...this.getPlugins(PluginType.before), ...this.getPlugins(PluginType.enrichment), ...this.getPlugins(PluginType.utility), ...this.getPlugins(PluginType.destination), ...this.getPlugins(PluginType.after)];
  }
  ...
  constructor({
    ...
    this.adTrackingEnabled = {
      get: createGetter(() => this.store.context.get()?.device?.adTrackingEnabled ?? false, async () => {
  ...
  async init() {
    try {
      ...
      this.reportInternalError(new SegmentError(ErrorType.InitializationError, 'Client did not initialize correctly', error));
    }
  }
  ...
  getEndpointForSettings() {
    ...
    if (hasProxy) {
      ...
      if (useSegmentEndpoints) {
        ...
        settingsEndpoint = isCdnProxyEndsWithSlash ? `projects/${this.config.writeKey}/settings` : `/projects/${this.config.writeKey}/settings`;
      }
    } else {
    ...
    try {
      ...
      console.error('Error in getEndpointForSettings:', `fallback to ${settingsCDN}/${this.config.writeKey}/settings`);
  ...
  */
  ...
  async flush() {
    try {
      ...
      this.reportInternalError(new SegmentError(ErrorType.FlushError, 'Flush failed', error));
    }
  }
  ...
  handleAppStateChange(nextAppState) {
    if (this.config.trackAppLifecycleEvents === true) {
      if (['inactive', 'background'].includes(this.appState) && nextAppState === 'active') {
        ...
        void this.process(event);
        this.logger.info('TRACK (Application Opened) event saved', event);
      } else if ((this.appState === 'active' || this.appState === 'unknown') &&
      // Check if appState is 'active' or 'unknown'
  ...
  async reset(resetAnonymousId = true) {
    try {
      ...
      await allSettled(getPluginsWithReset(this.timeline).map(plugin => plugin.reset()));
      this.logger.info('Client has been reset');
    } catch (error) {
      this.reportInternalError(new SegmentError(ErrorType.ResetError, 'Error during reset', error));
    }
  }
  ...
  setupFlushPolicies() {
    ...
    if (this.config.flushPolicies !== undefined) {
      ...
      if (this.config.flushAt === undefined || this.config.flushAt !== null && this.config.flushAt > 0) {
        flushPolicies.push(new CountFlushPolicy(this.config.flushAt ?? defaultFlushAt));
      }
      if (this.config.flushInterval === undefined || this.config.flushInterval !== null && this.config.flushInterval > 0) {
        flushPolicies.push(new TimerFlushPolicy((this.config.flushInterval ?? defaultFlushInterval) * 1000));
      }
    }
