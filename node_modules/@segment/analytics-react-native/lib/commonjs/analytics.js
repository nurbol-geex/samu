...
class SegmentClient {
  ...
  getEndpointForSettings() {
    ...
    if (hasProxy) {
      ...
      settingsPrefix = _constants.settingsCDN;
      settingsEndpoint = `/${this.config.writeKey}/settings`;
    }
    try {
      return (0, _util.getURL)(settingsPrefix, settingsEndpoint);
    } catch (error) {
      console.error('Error in getEndpointForSettings:', `fallback to ${_constants.settingsCDN}/${this.config.writeKey}/settings`);
      return `${_constants.settingsCDN}/${this.config.writeKey}/settings`;
    }
  }
  async fetchSettings() {
