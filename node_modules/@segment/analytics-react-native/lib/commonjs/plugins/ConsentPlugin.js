...
var _SegmentDestination = require("./SegmentDestination");
 ...
 */
class ConsentPlugin extends _plugin.Plugin {
  ...
  injectConsentFilterIfApplicable = plugin => {
    if (this.isDestinationPlugin(plugin)) {
      plugin.add(new ConsentFilterPlugin(event => {
        ...
        if (plugin.key === _SegmentDestination.SEGMENT_DESTINATION_KEY) {
  ...
  isDestinationPlugin(plugin) {
    return plugin.type === _types.PluginType.destination;
  }
...
class ConsentFilterPlugin extends _plugin.Plugin {
  type = _types.PluginType.before;
